...
> **本 Fork 说明**：这是 [earendil-works/pi](https://github.com/earendil-works/pi) 的非官方简体中文 Fork，当前基于上游 `v0.87.1`。默认使用简体中文，也可即时切换英文。汉化范围与构建方法见下方[汉化说明](#汉化说明)。
...
- `/session` 会话信息面板、缓存预热状态与中止原因、工具输出的折叠提示、分支与压缩摘要的折叠提示
- 崩溃提示与 `/bug` 建议、剪贴板报错、CLI 参数校验报错（`--mode`、`--tui-mode`、`--thinking`、未知选项等）
- 会话选择器的目录切换与空状态、会话树筛选标签、资源作用域标签、重试中止与排队消息状态
...
**尚未汉化**：`pi install/update/list` 等包管理子命令、`/share` 分享流程、`auth` 子命令的输出，以及实验功能（`experimental/` 下的 mini/micro TUI）的新增文案；全屏模式下的 `Copied!`/`Copy failed` 位于无文案层的 `packages/tui` 库内。这些不影响交互界面主流程。
